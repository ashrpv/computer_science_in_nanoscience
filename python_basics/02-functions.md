...
### Самопроверка после лекции 3

Почему `zip(ids, signals)` без `strict=True` может скрыть потерянное измерение? Чем отличаются `return` и `yield` в `calibrated_rows`? Что изменится при повторном обходе объекта генератора после его исчерпания? Какой дополнительный аргумент или проверку вы добавили бы, если калибровочный диапазон зависит от метода?

Закрепите базовые вызовы в [задачах 1–10](02-homework-a.md); чистую функцию и генератор — в [дополнительной практике](02-homework-a.md#дополнительная-самостоятельная-практика-поток-калибровочных-записей). Затем переходите к [дополнению об окружениях](modules_pypi_venv.md) и [лекции 4](04-numpy_and_visualization.md).
